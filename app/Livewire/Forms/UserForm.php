...
class UserForm extends Form
{
    ...
    public $city = '';
    public $address = '';
    public $role = User::ROLE_KARYAWAN;
    ...
    {
        $requiredOrNullable = $this->role === User::ROLE_KARYAWAN ? 'required' : 'nullable';
        return [
            'name' => [
                'required',
                'string',
            ...
            'status' => ['nullable', 'string', Rule::in([User::STATUS_ACTIVE, User::STATUS_INACTIVE])],
    ...
    private function isAllowed()
    {
        return Auth::user()?->isHrd
            || (Auth::user()?->isAdmin && Auth::user()?->id === $this->user?->id);
    }
}
